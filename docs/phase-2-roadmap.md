...
二期仍保持本地优先，不引入远程部署、多租户权限、无人值守自动更新、多实例编排和云端同步。远程集群管理可以进入后续版本评估。
...
| 模型下载任务锁与恢复 | 防止同一模型重复下载，支持中断后恢复 | 同一 `repoId + fileName` 仅有一个 active 任务；刷新页面后进度可恢复；传输中断后保留 partial 用于续传 |
...
- Hugging Face 下载增加 `已下载/总大小/ETA` 可视化，支持任务恢复展示。
